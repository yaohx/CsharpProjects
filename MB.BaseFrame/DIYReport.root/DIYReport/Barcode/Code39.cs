...
{
	class Code39 : BarcodeVariables, IBarcode {
		...
		private string Encode_Code39() {
			...
			foreach (char c in FormattedData) {
				try {
					...
					if (_AllowExtended)
						return string.Empty;//throw new Exception("EC39-1: Invalid data.");
					else
						return string.Empty;//throw new Exception("EC39-1: Invalid data. (Try using Extended Code39)");
				}//catch
			}//foreach
